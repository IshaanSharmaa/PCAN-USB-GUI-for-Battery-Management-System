...
PCAN_ERROR_XMTFULL = TPCANStatus(
    0x00001)  # Transmit buffer in CAN controller is full
PCAN_ERROR_OVERRUN = TPCANStatus(0x00002)  # CAN controller was read too late
PCAN_ERROR_BUSLIGHT = TPCANStatus(
    0x00004)  # Bus error: an error counter reached the 'light' limit
PCAN_ERROR_BUSHEAVY = TPCANStatus(
    0x00008)  # Bus error: an error counter reached the 'heavy' limit
PCAN_ERROR_BUSWARNING = TPCANStatus(
    PCAN_ERROR_BUSHEAVY
)  # Bus error: an error counter reached the 'warning' limit
PCAN_ERROR_BUSPASSIVE = TPCANStatus(
    0x40000)  # Bus error: the CAN controller is error passive
PCAN_ERROR_BUSOFF = TPCANStatus(
    0x00010)  # Bus error: the CAN controller is in bus-off state
...
PCAN_ERROR_NETINUSE = TPCANStatus(
    0x00800)  # A Client is already connected to the Net
...
PCAN_ERROR_RESOURCE = TPCANStatus(
    0x02000)  # Resource (FIFO, Client, timeout) cannot be created
...
PCAN_ERROR_ILLMODE = TPCANStatus(
    0x80000)  # Driver object state is wrong for the attempted operation
...
# PCAN devices
#
PCAN_NONE = TPCANDevice(
    0x00)  # Undefined, unknown or not selected PCAN device value
PCAN_PEAKCAN = TPCANDevice(
    0x01)  # PCAN Non-PnP devices. NOT USED WITHIN PCAN-Basic API
PCAN_ISA = TPCANDevice(0x02)  # PCAN-ISA, PCAN-PC/104, and PCAN-PC/104-Plus
PCAN_DNG = TPCANDevice(0x03)  # PCAN-Dongle
PCAN_PCI = TPCANDevice(
    0x04)  # PCAN-PCI, PCAN-cPCI, PCAN-miniPCI, and PCAN-PCI Express
PCAN_USB = TPCANDevice(0x05)  # PCAN-USB and PCAN-USB Pro
PCAN_PCC = TPCANDevice(0x06)  # PCAN-PC Card
PCAN_VIRTUAL = TPCANDevice(
    0x07)  # PCAN Virtual hardware. NOT USED WITHIN PCAN-Basic API
...
PCAN_RECEIVE_EVENT = TPCANParameter(
    0x03)  # PCAN receive event handler parameter
...
PCAN_CHANNEL_VERSION = TPCANParameter(
...
PCAN_LOG_CONFIGURE = TPCANParameter(
    0x0B)  # Configuration of the debugged information (LOG_FUNCTION_***)
PCAN_LOG_TEXT = TPCANParameter(
    0x0C)  # Custom insertion of text into the log file
PCAN_CHANNEL_CONDITION = TPCANParameter(
    0x0D)  # Availability status of a PCAN-Channel
PCAN_HARDWARE_NAME = TPCANParameter(0x0E)  # PCAN hardware name parameter
PCAN_RECEIVE_STATUS = TPCANParameter(
    0x0F)  # Message reception status of a PCAN-Channel
PCAN_CONTROLLER_NUMBER = TPCANParameter(
    0x10)  # CAN-Controller number of a PCAN-Channel
PCAN_TRACE_LOCATION = TPCANParameter(
    0x11)  # Directory path for PCAN trace files
PCAN_TRACE_STATUS = TPCANParameter(0x12)  # CAN tracing activation status
PCAN_TRACE_SIZE = TPCANParameter(
    0x13)  # Configuration of the maximum file size of a CAN trace
PCAN_TRACE_CONFIGURE = TPCANParameter(
    0x14)  # Configuration of the trace file storing mode (TRACE_FILE_***)
...
PCAN_CHANNEL_FEATURES = TPCANParameter(
    0x16)  # Capabilities of a PCAN device (FEATURE_***)
PCAN_BITRATE_ADAPTING = TPCANParameter(
    0x17)  # Using of an existing bit rate (PCAN-View connected to a channel)
PCAN_BITRATE_INFO = TPCANParameter(
    0x18)  # Configured bit rate as Btr0Btr1 value
PCAN_BITRATE_INFO_FD = TPCANParameter(
    0x19)  # Configured bit rate as TPCANBitrateFD string
PCAN_BUSSPEED_NOMINAL = TPCANParameter(
    0x1A)  # Configured nominal CAN Bus speed as Bits per seconds
PCAN_BUSSPEED_DATA = TPCANParameter(
    0x1B)  # Configured CAN data speed as Bits per seconds
PCAN_IP_ADDRESS = TPCANParameter(
    0x1C)  # Remote address of a LAN channel as string in IPv4 format
PCAN_LAN_SERVICE_STATUS = TPCANParameter(
    0x1D)  # Status of the Virtual PCAN-Gateway Service
PCAN_ALLOW_STATUS_FRAMES = TPCANParameter(
    0x1E)  # Status messages reception status within a PCAN-Channel
PCAN_ALLOW_RTR_FRAMES = TPCANParameter(
    0x1F)  # RTR messages reception status within a PCAN-Channel
PCAN_ALLOW_ERROR_FRAMES = TPCANParameter(
    0x20)  # Error messages reception status within a PCAN-Channel
PCAN_INTERFRAME_DELAY = TPCANParameter(
    0x21)  # Delay, in microseconds, between sending frames
PCAN_ACCEPTANCE_FILTER_11BIT = TPCANParameter(
    0x22)  # Filter over code and mask patterns for 11-Bit messages
PCAN_ACCEPTANCE_FILTER_29BIT = TPCANParameter(
    0x23)  # Filter over code and mask patterns for 29-Bit messages
...
PCAN_IO_DIGITAL_VALUE = TPCANParameter(
    0x25)  # Value assigned to a 32 digital I/O pins of a PCAN-USB Chip
...
PCAN_IO_DIGITAL_CLEAR = TPCANParameter(
    0x27)  # Clear multiple digital I/O pins to 0
PCAN_IO_ANALOG_VALUE = TPCANParameter(
    0x28)  # Get value of a single analog input pin
...
PCAN_ATTACHED_CHANNELS_COUNT = TPCANParameter(
    0x2A)  # Get the amount of PCAN channels attached to a system
PCAN_ATTACHED_CHANNELS = TPCANParameter(
    0x2B)  # Get information about PCAN channels attached to a system
PCAN_ALLOW_ECHO_FRAMES = TPCANParameter(
    0x2C)  # Echo messages reception status within a PCAN-Channel
PCAN_DEVICE_PART_NUMBER = TPCANParameter(
    0x2D)  # Get the part number associated to a device
...
PCAN_FILTER_CLOSE = int(
    0x00)  # The PCAN filter is closed. No messages will be received
...
LOG_FUNCTION_ENTRY = int(
    0x01)  # Logs the entries to the PCAN-Basic API functions
LOG_FUNCTION_PARAMETERS = int(
    0x02)  # Logs the parameters passed to the PCAN-Basic API functions
LOG_FUNCTION_LEAVE = int(
    0x04)  # Logs the exits from the PCAN-Basic API functions
LOG_FUNCTION_WRITE = int(
    0x08)  # Logs the CAN messages passed to the CAN_Write function
LOG_FUNCTION_READ = int(
    0x10)  # Logs the CAN messages received within the CAN_Read function
...
TRACE_FILE_DATE = int(
    0x02)  # Includes the date into the name of the trace file
TRACE_FILE_TIME = int(
    0x04)  # Includes the start time into the name of the trace file
...
PCAN_MESSAGE_STANDARD = TPCANMessageType(
    0x00)  # The PCAN message is a CAN Standard Frame (11-bit identifier)
PCAN_MESSAGE_RTR = TPCANMessageType(
    0x01)  # The PCAN message is a CAN Remote-Transfer-Request Frame
PCAN_MESSAGE_EXTENDED = TPCANMessageType(
    0x02)  # The PCAN message is a CAN Extended Frame (29-bit identifier)
PCAN_MESSAGE_FD = TPCANMessageType(
    0x04)  # The PCAN message represents a FD frame in terms of CiA Specs
...
PCAN_MESSAGE_ECHO = TPCANMessageType(
    0x20)  # The PCAN message represents an echo CAN Frame
PCAN_MESSAGE_ERRFRAME = TPCANMessageType(
    0x40)  # The PCAN message represents an error frame
PCAN_MESSAGE_STATUS = TPCANMessageType(
    0x80)  # The PCAN message represents a PCAN status message
...
class TPCANChannelInformation(Structure):
    ...
    _fields_ = [
        ...
        ("device_features",
         c_uint),  # Device capabilities flag (see FEATURE_*)
...
class PCANBasic:
    ...
    def __init__(self):
        ...
        if platform.system() == 'Windows':
            ...
            # is a third-party software creaded and mantained by the MacCANject. For
    ...
    def Initialize(self,
        ...
        """
        try:
            res = self.__m_dllBasic.CAN_Initialize(Channel, Btr0Btr1, HwType,
                                                   IOPort, Interrupt)
    ...
    def Read(self, Channel):
        ...
        try:
            ...
            res = self.__m_dllBasic.CAN_Read(Channel, byref(msg),
                                             byref(timestamp))
    ...
    def ReadFD(self, Channel):
        ...
        try:
            ...
            res = self.__m_dllBasic.CAN_ReadFD(Channel, byref(msg),
                                               byref(timestamp))
            return TPCANStatus(res), msg, timestamp
        except:
    ...
    def FilterMessages(self, Channel, FromID, ToID, Mode):
        ...
        """
        try:
            res = self.__m_dllBasic.CAN_FilterMessages(Channel, FromID, ToID,
                                                       Mode)
            return TPCANStatus(res)
        except:
    ...
    def GetValue(self, Channel, Parameter):
        ...
        try:
            ...
            res = self.__m_dllBasic.CAN_GetValue(Channel, Parameter,
                                                 byref(mybuffer),
                                                 sizeof(mybuffer))
    ...
    def SetValue(self, Channel, Parameter, Buffer):
        ...
        try:
            ...
            res = self.__m_dllBasic.CAN_SetValue(Channel, Parameter,
                                                 byref(mybuffer),
                                                 sizeof(mybuffer))
            return TPCANStatus(res)
        except:
    ...
    def GetErrorText(self, Error, Language=0):
        ...
        try:
            mybuffer = create_string_buffer(256)
            res = self.__m_dllBasic.CAN_GetErrorText(Error, Language,
                                                     byref(mybuffer))
            return TPCANStatus(res), mybuffer.value
        except:
    ...
    def LookUpChannel(self, Parameters):
        ...
        try:
            ...
            res = self.__m_dllBasic.CAN_LookUpChannel(Parameters,
                                                      byref(mybuffer))
            return TPCANStatus(res), mybuffer
        except:
