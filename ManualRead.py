...
class ManualRead():
    ...
    def __init__(self):
        ...
        if self.IsFD:
            stsResult = self.m_objPCANBasic.InitializeFD(
                self.PcanHandle, self.BitrateFD)
        else:
            stsResult = self.m_objPCANBasic.Initialize(self.PcanHandle,
                                                       self.Bitrate)
        ...
        while strinput == "y":
            ...
            self.ReadMessages()
            print(
                "Do you want to read again? yes[y] or any other key to close")
    ...
    def FormatChannelName(self, handle, isFD=False):
        ...
        if isFD:
            return (
                '%s:FD %s (%.2Xh)' %
                (self.GetDeviceName(devDevice.value), byChannel, handleValue))
        else:
            return (
                '%s %s (%.2Xh)' %
                (self.GetDeviceName(devDevice.value), byChannel, handleValue))
        ...
        if (msgtype & PCAN_MESSAGE_RTR.value) == PCAN_MESSAGE_RTR.value:
            ...
            if (msgtype > PCAN_MESSAGE_EXTENDED.value):
                ...
                if (msgtype
                    ...
                    strTemp += ' BRS'
                if (msgtype
                        & PCAN_MESSAGE_ESI.value) == PCAN_MESSAGE_ESI.value:
                    strTemp += ' ESI'
                strTemp += ' ]'
